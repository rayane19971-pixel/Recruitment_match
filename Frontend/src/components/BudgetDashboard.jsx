...
export default function BudgetDashboard({ token, role }) {
  ...
  useEffect(() => {
    if (role === 'scout') {
      setError("Votre rôle (scout) ne vous permet pas d'accéder à la gestion budgétaire confidentielle de l'Olympique Lyonnais.");
      return;
    }
    ...
    fetch('http://127.0.0.1:8000/director/budget', {
      ...
      .catch(err => {
        setBudgetData({
          message: "Accès autorisé aux données financières de la direction OL.",
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div className="glass-card" style={{ borderLeft: '5px solid #d31115' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#e5a93c', marginBottom: '0.5rem' }}>
          💰 Espace Direction Sportive & Budget Mercato OL 🔴🔵
        </h2>
        <p style={{ color: '#94a3b8', fontSize: '0.9rem' }}>
          Suivi confidentiel des enveloppes de transfert et simulation de masse salariale de l'Olympique Lyonnais.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
        <div className="glass-card" style={{ borderLeft: '4px solid #10b981' }}>
          <span style={{ color: '#94a3b8', fontSize: '0.8rem', textTransform: 'uppercase' }}>Enveloppe Transferts OL</span>
          <h3 style={{ fontSize: '2rem', fontWeight: 900, color: '#10b981', margin: '0.25rem 0' }}>45 000 000 €</h3>
          <span style={{ color: '#64748b', fontSize: '0.75rem' }}>Saison 2026 - Validé par la Direction OL</span>
        </div>
      ...
      <div className="glass-card">
        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem', color: '#f8fafc' }}>
          📊 Simulateur d'Impact Financier d'un Recrutement OL :
        </h3>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', padding: '12px 16px', borderRadius: '8px', fontSize: '0.85rem' }}>
          <strong>Analyse de viabilité OL :</strong> Un transfert de <strong>{(simulatedTransfer / 1000000).toFixed(1)} M€</strong> avec un salaire de <strong>{(simulatedSalary / 1000000).toFixed(2)} M€/an</strong> consomme <strong>{((simulatedTransfer / envelopeNum) * 100).toFixed(1)}%</strong> de l'enveloppe mercantiles totale du club.
        </div>
      </div>
    </div>
  );
